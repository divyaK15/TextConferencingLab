                        ...
                        char buf_cpy[MESSAGE_SIZE] = {'\0'};
                            ...
                            if (recv_message.type == LOGIN){
                                ...
                                if (sessionExists(recv_message.data)){
                                    // iterate through client list until we find the client with the same (unique) username as the current one?
                                    // if the client is the same, update their session to be the session they want to join
                                    // send JN_ACK to client
                                    int user = 0;
                                    for (user = 0; (user < MAX_USERS) && (g_masterClientList[user] != NULL); ++user){
                                        client_info* current_client = g_masterClientList[user];
                                        if (strcmp(current_client->username, recv_message.source) == 0){
                                            strcpy(current_client->current_session, recv_message.data);
                                            if (send(i, "JN_ACK", sizeof("JN_ACK"), 0) < 0){
                                                printf("i hate it here");
                                            }
                                            break;
                                        }
                                    }
                                    ...
                                    // send a JN_NAK to client 
                                    if (send(i, "JN_NAK" , sizeof("JN_NAK"), 0) < 0){
                                        printf("i hate it here pt 2");
                                    }
                                }
